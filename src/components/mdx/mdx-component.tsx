...
import TechMdx from '../tech-icons';
...
const components = {
  ...
  blockquote: ({ className, ...props }: React.HtmlHTMLAttributes<HTMLQuoteElement>) => (
    <blockquote
      className={cn('mt-6 border-l-2 pl-6 italic [&>*]:text-accent', className)}
      {...props}
    />
  ),
  ...
  pre: ({ className, ...props }: React.HtmlHTMLAttributes<HTMLPreElement>) => (
    <pre
      className={cn(
        'mb-4 mt-6 overflow-x-auto rounded-lg bg-black bg-[url(/images/noise.png)] p-4 [&>code]:bg-transparent',
        className,
      )}
      {...props}
    />
  ...
  code: ({ className, ...props }: React.HtmlHTMLAttributes<HTMLElement>) => (
    <code
      className={cn(
        'relative rounded bg-primary/10 px-[0.3rem] py-[0.2rem] font-mono text-sm text-primary',
        className,
      )}
      {...props}
    />
